...
def nächsteZeiteinheit(Autobahn, p, vmax, Geschwindigkeitsbegrenzung):
    ...
    for i in range(len(Autobahn)):
        if Autobahn[i][0] != 0:
            ...
            if (i + Autobahn[i][0].getV()) >= len(Autobahn):
                neueS[(i + Autobahn[i][0].getV()) % len(Autobahn)][0] = Autobahn[i][0]
                continue
            else:
...
def neueGeschwindigkeitsbegrenzung(Autobahn, GLS):
    ...
    return ([Höchstgeschwindigkeit(b, GLS), Höchstgeschwindigkeit(c, GLS), Höchstgeschwindigkeit(d, GLS), Höchstgeschwindigkeit(e, GLS), Höchstgeschwindigkeit(f, GLS), Höchstgeschwindigkeit(g, GLS), Höchstgeschwindigkeit(a, GLS)])
...
def restlicheDaten():#weniger als 35 min
...
def Heatmap():#zur grafischen Darstellung der Schockwellen/NaSchModell Grafik
    Geschwindigkeitsbegrenzung = [10, 10, 10, 10, 10, 10, 10]
    vmax = 7  # Höchstgeschwindigkeit der Fahrzeuge
    p = [0.22, 0.2, 0.16, 0.13, 0.11, 0.08, 0.06, 0.05]  # Wahrscheinlichkeit, dass ein Auto trödelt
    a = 200  # 1/7 der Zellen
    ...
    OrtZeitÜbersicht = [[] for x in range(601)]
    count = 0
    OrtZeitÜbersicht[count] = Autobahn
    while count < 600:
        Geschwindigkeitsbegrenzung = neueGeschwindigkeitsbegrenzung(Autobahn, GLS)
        Autobahn = GeschwindigkeitsbegrenzungEinführen(Autobahn, Geschwindigkeitsbegrenzung)
        Autobahn = nächsteZeiteinheit(Autobahn, p, vmax, Geschwindigkeitsbegrenzung)
        count += 1
        OrtZeitÜbersicht[count] = Autobahn
    DichteBild = ["" for x in range(int(len(OrtZeitÜbersicht)))]
    for i in range(len(DichteBild)):
        Dichte = [AutosZählen(OrtZeitÜbersicht[i], x * 5, (x + 1) * 5 - 1) / 5 for x in range(len(Autobahn) // 5)]
        DichteBild[i] = Dichte

    fig, ax = plt.subplots()
    q = ax.imshow(DichteBild, cmap='hot', interpolation='nearest')
    ax.set_title('Density depending on time and place')
    ax.set_xlabel("Place")
    ax.set_ylabel("Time")
